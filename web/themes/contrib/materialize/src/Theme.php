...
class Theme {
  ...
  public function __construct(Extension $theme, ThemeHandlerInterface $theme_handler) {
    ...
    $this->dev = (bool) Settings::get('theme.dev');
  ...
  /**
   * Wrapper for the FileSystemInterface::scanDirectory() function.
   *
   ...
   * @param array $options
   *   Options to pass, see FileSystemInterface::scanDirectory() for
   *   addition options:
   ...
   *
   * @see FileSystemInterface::scanDirectory()
   */
  public function fileScan($mask, $subdir = NULL, array $options = []) {
    ...
    if (!$files->has($hash)) {
      ...
      $current_files = $fileSystem->scanDirectory($path, $mask, $options);
      $files->set($hash, $current_files);
    }
